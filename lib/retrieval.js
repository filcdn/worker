...
const debug = createDebug('filcdn:worker:retrieve')
 ...
 * @returns {Promise<{
 *   response: Response
 *   cacheMiss: undefined | boolean
 *   contentLength: undefined | string
 * }>}
...
export async function retrieveFile(baseUrl, pieceCid, cacheTtl = 86400) {
  ...
  if (!cacheMiss || !contentLength) {
    debug(
      'Response headers cacheMiss: %s or contentLength: %s are not available for pieceCid=%s and hostname=%s',
      cacheMiss,
      contentLength,
      pieceCid,
      baseUrl,
