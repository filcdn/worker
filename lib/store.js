 ...
 */
export async function logRetrievalResult (env, { hostname, pieceCid, response, cacheMiss, proofSetId }) {
  const timestamp = new Date().toISOString()
  const responseStatus = response.status
  const contentLength = response.headers.get('content-length')
  if (!contentLength) {
    debug('No content-length header found in response for pieceCid: and hostname:', pieceCid, hostname)
  }
  ...
  try {
    await env.DB.prepare(`
        INSERT INTO retrieval_logs 
          (timestamp, hostname, piece_cid, response_status, egress_bytes, cache_miss, proof_set_id)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `).bind(
      timestamp,
      hostname,
