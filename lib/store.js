 ...
 */
export async function logRetrievalResult (env, { ownerAddress, clientAddress, response, cacheMiss, timestamp }) {
  const responseStatus = response.status
  const contentLength = response.headers.get('Content-Length')
  if (!contentLength) {
    debug('No content-length header found in response for pieceCid=%s and hostname=%s', ownerAddress, clientAddress)
  }
