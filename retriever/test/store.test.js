...
describe('getOwnerAndValidateClient', () => {
  ...
  it('excludes sanctioned clients from approved providers selection', async () => {
    ...
    const unsanctionedClientAddress = '0x2222def1234567890abcdef1234567890abcd2222'  
    ...
    // Add the sanctioned client to the sanctions table
    await env.DB.prepare(
      'INSERT INTO sanctioned_clients (client_address) VALUES (?)'
    )
      .bind(sanctionedClientAddress.toLowerCase())
      .run()
    ...
    await env.DB.batch([
      ...
      env.DB.prepare(
        'INSERT INTO indexer_proof_set_rails (proof_set_id, rail_id, payer, payee, with_cdn) VALUES (?, ?, ?, ?, ?)'
      ).bind(proofSetId, 'rail-sanctioned', sanctionedClientAddress, ownerAddress, true),
      env.DB.prepare(
        'INSERT INTO indexer_proof_set_rails (proof_set_id, rail_id, payer, payee, with_cdn) VALUES (?, ?, ?, ?, ?)'
      ).bind(proofSetId, 'rail-unsanctioned', unsanctionedClientAddress, ownerAddress, true),
    ])
  
    // Test 1: Sanctioned client should be rejected
    await assert.rejects(
      async () => await getOwnerAndValidateClient(env, sanctionedClientAddress, rootCid),
      /sanctioned/i,
      'Should reject sanctioned client'
    )
  
    // Test 2: Unsanctioned client should work normally
    const result = await getOwnerAndValidateClient(env, unsanctionedClientAddress, rootCid)
    assert.strictEqual(result.ownerAddress, ownerAddress.toLowerCase())
    assert.strictEqual(result.proofSetId, proofSetId)
    assert.strictEqual(result.pieceRetrievalUrl, 'https://approved-provider.xyz')
  })
})
