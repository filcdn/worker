 ...
 * @returns {Promise<
 *   { address: string; status: 'sanctioned' | 'approved' | 'pending' }[]
 * >}
 */
export async function checkAddresses(
  ...
  for (const address of validAddresses) {
    const sanctionCheck = await fetchChainalysisSanctions(address, apiKey, {
      fetch,
    })
 ...
 * }>}
 */
async function fetchChainalysisSanctions(
  address,
  apiKey,
  { fetch = globalThis.fetch } = {},
) {
