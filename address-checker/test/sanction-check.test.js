...
describe('checkAddresses', () => {
  ...
  it('returns empty array when no valid addresses are provided', async () => {
    // Execute
    const result = await checkAddresses(['invalid1', 'invalid2'], apiKey, {
      fetch: mockFetch,
    })
  ...
  it('correctly identifies sanctioned addresses', async () => {
    ...
    // Execute
    const results = await checkAddresses(addresses, apiKey, {
      fetch: mockFetch,
    })
  ...
  it('handles API errors by marking address as pending', async () => {
    ...
    // Verify
    expect(results).toEqual([{ address: address[0], status: 'pending' }])
  })
