...
describe('getAddressesToCheck', () => {
  it('retrieves pending addresses from the database', async () => {
    ...
    assert.ok(
      pendingAddresses.includes('0x1111111111111111111111111111111111111111'),
    )
    assert.ok(
      pendingAddresses.includes('0x2222222222222222222222222222222222222222'),
    )
    assert.ok(
      !pendingAddresses.includes('0x3333333333333333333333333333333333333333'),
    )
    assert.ok(
      !pendingAddresses.includes('0x4444444444444444444444444444444444444444'),
    )
  })
...
describe('updateAddressStatuses', () => {
  it('updates the status of existing addresses', async () => {
    ...
    const addr1 = results.results.find(
      (r) => r.address === '0xaaaa111111111111111111111111111111111111',
    )
    const addr2 = results.results.find(
      (r) => r.address === '0xbbbb222222222222222222222222222222222222',
    )
  ...
  it('inserts new addresses that do not exist yet', async () => {
    ...
    await env.DB.prepare('DELETE FROM address_sanction_check WHERE address = ?')
      .bind('0xcccc333333333333333333333333333333333333')
      .run()
...
describe('addMissingAddresses', () => {
  it('adds addresses from indexer_proof_set_rails that are not in address_sanction_check', async () => {
    ...
    await env.DB.prepare(
      ...
      .bind('0x1111111111111111111111111111111111111111', 'approved')
      .run()
    ...
    const preExisting = results.results.find(
      (r) => r.address === '0x1111111111111111111111111111111111111111',
    )
