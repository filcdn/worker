...
import {
  sumEgressPerOwnerAfterTimestamp,
  calculateRewardsPerOwner,
  getCurrentOwnerRewards,
} from './utils.js'
...
export async function handleRewardsCalculation(env) {
  const egressByOwner = await sumEgressPerOwnerAfterTimestamp(
    env,
    new Date().toISOString(),
  )
  const maxEgressPerOwner = await calculateMaxRetrievalEgressPerOwner(env)
  const rewardsPerOwner = calculateRewardsPerOwner(
    egressByOwner,
    maxEgressPerOwner,
  )
  await updateOwnerRewards(env, rewardsPerOwner)
  return new Response('Rewards calculation completed successfully', {
    status: 200,
  })
}
...
export async function handleFetchOwnerRewards(env) {
  const rewardsPerOwner = await getCurrentOwnerRewards(env)
